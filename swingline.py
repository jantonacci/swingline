...
import re, os, gzip, time, sys, logging, argparse, shutil
import dataset, xlsxwriter
...
def main():
    ...
    cat_dict = dict(storage='date,time,category,host,dev,dsname,latency,world,cmd,t10,sense,asense,raw',
                    latency='date,time,host,dev,dsname,latency,raw',
                    iofails='date,time,host,dev,dsname,world,cmd,t10,sense,raw',
                    sioclmt='date,time,host,dev,dsname,world,cmd,t10,raw',
                    apdpdls='date,time,host,dev,dsname,raw',
                    order='date,time,host')

    sql_dict = dict(storage='SELECT DISTINCT {fields} FROM {table} ORDER BY {order}'.format(table='storage', fields=cat_dict['storage'], order=cat_dict['order']),
                    latency='SELECT DISTINCT {fields} FROM {table} WHERE category=\'{cat}\' ORDER BY {order}'.format(table='storage', fields=cat_dict['latency'], cat='latency', order=cat_dict['order']),
                    iofails='SELECT DISTINCT {fields} FROM {table} WHERE category=\'{cat}\' ORDER BY {order}'.format(table='storage', fields=cat_dict['iofails'], cat='iofails', order=cat_dict['order']),
                    sioclmt='SELECT DISTINCT {fields} FROM {table} WHERE category=\'{cat}\' ORDER BY {order}'.format(table='storage', fields=cat_dict['sioclmt'], cat='sioclmt', order=cat_dict['order']),
                    apdpdls='SELECT DISTINCT {fields} FROM {table} WHERE category=\'{cat}\' ORDER BY {order}'.format(table='storage', fields=cat_dict['apdpdls'], cat='apdpdls', order=cat_dict['order']))
...
def set_opt_default():
    ...
    opt_dict.update({'csv_file': '{dirpath}/swingline{unique}.csv'.format(dirpath=opt_dict['srdata'], unique=opt_dict['tstamp']),
                     ...
                     'summary_file': '{dirpath}/swingline{unique}.txt'.format(dirpath=opt_dict['srdata'], unique=opt_dict['tstamp'])})
...
def set_parser(opt_dict):
    ...
    parser.add_argument('-b', '--bundle_dir',
                        ...
                        action='store',
                        dest='bundle_dir',
                        help=': vm-support bundle extracted directory - default is "{dir}"'.format(dir=opt_dict['srdata']))
    ...
    parser.add_argument('-t', '--tmp_dir',
                        ...
                        action='store',
                        dest='tmp_dir',
                        help=': create temporary file(s) in directory - default is "{dir}"'.format(dir=opt_dict['tmp_dir']))
...
def parse_opt_export(arg_dict, opt_dict):
    ...
    if arg_dict.export and not 'default' in arg_dict.export:
        if 'all' in arg_dict.export.lower():
            ...
            RUNTIME_LOG.warning('Invalid arg_dict.export arguement "{arg}" - using default'.format(arg=arg_dict.export.lower()))
...
def parse_opt_bundle(arg_dict, opt_dict):
    ...
    if arg_dict.bundle_dir and not 'default' in arg_dict.bundle_dir:
        if os.path.exists(arg_dict.bundle_dir):
            ...
            RUNTIME_LOG.error('Invalid vm-support bundle extracted directory arguement "{arg}"'.format(arg=arg_dict.bundle_dir))
            sys.exit(1)

    if arg_dict.rpt_dir and not 'default' in arg_dict.rpt_dir:
        if os.path.exists(arg_dict.rpt_dir):
            opt_dict.update({'csv_file': '{dirpath}/swingline{unique}.csv'.format(dirpath=os.path.abspath(arg_dict.rpt_dir), unique=opt_dict['tstamp']),
                             'json_file': '{dirpath}/swingline{unique}.json'.format(dirpath=os.path.abspath(arg_dict.rpt_dir), unique=opt_dict['tstamp']),
                             'xlsx_file': '{dirpath}/swingline{unique}.xlsx'.format(dirpath=os.path.abspath(arg_dict.rpt_dir), unique=opt_dict['tstamp']),
                             'summary_file': '{dirpath}/swingline{unique}.txt'.format(dirpath=os.path.abspath(arg_dict.rpt_dir), unique=opt_dict['tstamp'])})
        else:
            RUNTIME_LOG.warning('Invalid reports directory arguement "{arg}" - using default'.format(arg=arg_dict.rpt_dir))
...
def parse_opt_temp(arg_dict, opt_dict):
    ...
    if arg_dict.tmp_dir and not 'default' in arg_dict.tmp_dir:
        if os.path.exists(arg_dict.log_dir):
            ...
            RUNTIME_LOG.warning('Invalid temporary directory arguement "{arg}" - using default'.format(arg=arg_dict.tmp_dir))
...
def parse_opt_logging(arg_dict, opt_dict):
    ...
    if arg_dict.log_dir and not 'default' in arg_dict.log_dir:
        if os.path.exists(arg_dict.log_dir):
            ...
            logfile_vdiag = logging.handlers.TimedRotatingFileHandler('{path}/{file}'.format(path=os.path.abspath(arg_dict.log_dir), file='swingline.log'),
                                                                      when='d',
                                                                      interval=1,
                                                                      backupCount=7,
                                                                      delay=False,
            ...
            RUNTIME_LOG.debug('vDiag environment variable VDIAG_LOGDIR overrides defaults - new log file path "{path}"'.format(path=os.path.abspath(arg_dict.log_dir)))
        else:
            RUNTIME_LOG.warning('Invalid vm-support bundle extracted directory arguement "{arg}" - using default'.format(arg=arg_dict.bundle_dir))
...
def insert_sample():
    ...
    TBL_STORAGE.insert(dict(category='iofails', date='1970-01-01', hour='00', time='00:00:00.000Z', host='example.local', dev='naa.0123456789abcdef0123456789abcdef', dsname='SampleDatastore', world='0', cmd='0xff', t10='T10_XLATE', sense='H:GOOD D:GOOD P:GOOD', asense='H:0x0 D:0x0 P:0x0 Valid sense data: 0x0 0x0 0x0', raw='VMW KB 289902: Interpreting SCSI sense codes in VMware ESXi and ESX, http://kb.vmware.com/kb/289902'))
    TBL_STORAGE.insert(dict(category='sioclmt', date='1970-01-01', hour='00', time='00:00:00.000Z', host='example.local', dev='naa.0123456789abcdef0123456789abcdef', dsname='SampleDatastore', world='0', cmd='0xff', t10='T10_XLATE', raw='VMW KB 1038241: Limiting disk I/O from a specific virtual machine, http://kb.vmware.com/kb/1038241'))
    TBL_STORAGE.insert(dict(category='latency', date='1970-01-01', hour='00', time='00:00:00.000Z', host='example.local', dev='naa.0123456789abcdef0123456789abcdef', dsname='SampleDatastore', latency='0', raw='VMW KB 2007236: Storage device performance deteriorated, http://kb.vmware.com/kb/2007236'))
    TBL_STORAGE.insert(dict(category='apdpdls', date='1970-01-01', hour='00', time='00:00:00.000Z', host='example.local', dev='naa.0123456789abcdef0123456789abcdef', dsname='SampleDatastore', world='0', cmd='(ALL)', t10='(ALL)', latency='APD/PDL', raw='VMW KB 2004684: Permanent Device Loss (PDL) and All-Paths-Down (APD) in vSphere 5.x, http://kb.vmware.com/kb/2004684'))
...
def pop_db(path):
    ...
    pat_dict = {'dns': re.compile(r'^uname_-a.txt'),
                'txt': re.compile(r'^(vmkernel|vobd)(\.log|\.all|\.[0-9]+(?!\.gz))'),
                'gz': re.compile(r'^vmkernel\.[0-9]+\.gz'),
...
def parse_file(esxi_dict, pat_dict):
    ...
    if (pat_dict['dns'].search(esxi_dict['name']) or pat_dict['txt'].search(esxi_dict['name']) or pat_dict['gz'].search(esxi_dict['name']) or pat_dict['vmfs'].search(esxi_dict['name'])) and pat_dict['subdir'].search(os.path.abspath(os.path.join(esxi_dict['root'], esxi_dict['name']))) and not os.path.islink(os.path.abspath(os.path.join(esxi_dict['root'], esxi_dict['name']))):
        # Assign variable 'bundle' the vm-support directory name for later logging
        esxi_dict['bundle'] = re.sub(r'^.*/esx-', 'esx-', os.path.abspath(os.path.join(esxi_dict['root'], esxi_dict['name'])).lower())
        esxi_dict['bundle'] = re.sub(r'/.*$', '', esxi_dict['bundle'])
        esxi_dict['alt'] = re.sub(r'.*(esx-.*-[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]--[0-9][0-9]\.[0-9][0-9]).*', r'\1', os.path.abspath(os.path.join(esxi_dict['root'], esxi_dict['name'])).lower())
        esxi_dict['alt'] = re.sub(r'(esx-|-[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]--[0-9][0-9]\.[0-9][0-9])', '', esxi_dict['alt'])

        RUNTIME_LOG.debug('Processing vm-support "{dirpath}" - file "{file}"'.format(file=esxi_dict['name'], dirpath=esxi_dict['bundle']))
        ...
        if pat_dict['dns'].search(esxi_dict['name']):
...
def parse_file_dns(esxi_dict):
    ...
    with open(os.path.abspath(os.path.join(esxi_dict['root'], esxi_dict['name'])), "r") as fileopen:
    ...
    if not esxi_dict['uname']:
        RUNTIME_LOG.debug('Missing vm-support bundle "{dirpath}" uname file "{file} - using hostname "{host}"'.format(host=esxi_dict['alt'], dirpath=esxi_dict['bundle'], file='uname_-a.txt'))
        ...
        for line in fileopen:
            if not pat_dict['header'].search(line):
                ...
                if extent and datastore:
                    ...
                    RUNTIME_LOG.debug('Missing information from vm-support "{dirpath}" - vmfs extent file "{file}'.format(dirpath=esxi_dict['bundle'], file='localcli_storage-vmfs-extent-list.txt'))
    return esxi_dict
...
def parse_file_gz(esxi_dict):
    ...
    fileopen = gzip.GzipFile(os.path.abspath(os.path.join(esxi_dict['root'], esxi_dict['name'])), "r")
    for line in fileopen:
...
def insert_rec(esxi_dict, line):
    ...
    pat_dict = {'datetime': re.compile(r'[0-9]{4}-[0-9][0-9]-[0-9][0-9]T[0-9][0-9]:[0-9][0-9]:[0-9][0-9]\.[0-9]{3}Z'),
                ...
                'iofails': re.compile(r'ScsiDeviceIO\: [0-9]+\: Cmd\(0x[0-9a-fA-F]+\) 0x[0-9a-fA-F][0-9a-fA-F], CmdSN 0x[0-9a-fA-F]+ from world [0-9]+ to dev .* failed .* sense data: '),
    ...
    if pat_dict['datetime'].match(line):
        ...
        if pat_dict['apdpdls'].search(line):
            ...
            try:
                TBL_STORAGE.insert(dict(category=msg_dict['cat'], host=esxi_dict['uname'], date=msg_dict['date'], hour=msg_dict['hour'], time=msg_dict['time'], world=msg_dict['world'], cmd=msg_dict['cmd'], t10=msg_dict['t10'], dev=msg_dict['ext'], dsname=msg_dict['dsn'], latency=msg_dict['msec'], raw=line))
            except RuntimeError:
            ...
            msg_dict.update({'cat': 'iofails',
                             'cmd': re.sub(r'^.* Cmd\(0x[0-9a-fA-F]+\) |, CmdSN 0x[0-9a-fA-F]+ from world [0-9]+ to dev .*$', '', line),
            ...
            try:
                TBL_STORAGE.insert(dict(category=msg_dict['cat'], host=esxi_dict['uname'], date=msg_dict['date'], hour=msg_dict['hour'], time=msg_dict['time'], cmd=msg_dict['cmd'], t10=msg_dict['t10'], world=msg_dict['world'], dev=msg_dict['ext'], dsname=msg_dict['dsn'], sense=msg_dict['sense'], asense=msg_dict['asense'], raw=line))
            except RuntimeError:
                RUNTIME_LOG.warning('Failed creating record from message "{msg}[...]"'.format(msg=line[36:116]))

        elif pat_dict['latency'].search(line):
            # From OpenGrok.eng.vmware.com xref: /vsphere60u1.perforce/vsphere60u1/vmkernel/storage/device/scsi_device_io.c
            ...
            try:
                TBL_STORAGE.insert(dict(category=msg_dict['cat'], host=esxi_dict['uname'], date=msg_dict['date'], hour=msg_dict['hour'], time=msg_dict['time'], dev=msg_dict['ext'], dsname=msg_dict['dsn'], latency=msg_dict['msec'], raw=line))
            except RuntimeError:
                RUNTIME_LOG.warning('Failed creating record from message "{msg}[...]"'.format(msg=line[36:116]))
            ...
            try:
                TBL_STORAGE.insert(dict(category=msg_dict['cat'], host=esxi_dict['uname'], date=msg_dict['date'], hour=msg_dict['hour'], time=msg_dict['time'], cmd=msg_dict['cmd'], t10=msg_dict['t10'], world=msg_dict['world'], dev=msg_dict['ext'], dsname=msg_dict['dsn'], raw=line))
            except RuntimeError:
                RUNTIME_LOG.warning('Failed creating record from message "{msg}[...]"'.format(msg=line[36:116]))
...
def check_db():
    ...
    for tbl_name in DB.tables:
        ...
        if tbl_len > 4:
            RUNTIME_LOG.info('Captured dataset - table "{table}", records "{count}"'.format(table=tbl_name, count=tbl_len))
        else:
            RUNTIME_LOG.warning('Empty dataset - table "{table}", records "{count}" (should be > 4 records)'.format(table=tbl_name, count=tbl_len))
            RUNTIME_LOG.info('Empty dataset situation 1 - No block storage performance events exist (storage is NFS or local "mpx.C#.T#.L#"?)')
            RUNTIME_LOG.info('Empty dataset situation 2 - ESXi vm-support bundle file or directory permissions (wrx on vm-bundle?)')
...
def relocate_file(file_src, file_dst):
    ...
    if file_exists:
        ...
        if file_src == file_dst:
            RUNTIME_LOG.info('Moved results to "{dst}" - exists {exists}, size {size}'.format(dst=file_dst, exists=str(file_exists).lower(), size=os.path.getsize(file_dst)))
        else:
            try:
                shutil.move(file_src, file_dst)
                RUNTIME_LOG.info('Moved results to "{dst}" - exists {exists}, size {size}'.format(dst=file_dst, exists=str(file_exists).lower(), size=os.path.getsize(file_dst)))
            except RuntimeError:
                RUNTIME_LOG.info('Failed moving dataset freeze - temporary file "{src}" exists {exist}, destination file "{dst}" exists false'.format(src=file_src, exists=str(file_exists).lower(), dst=file_dst))
    else:
...
def freeze_summary(freeze_setup):
    ...
    freeze_export_tmp = '{path}/{file}'.format(path=freeze_setup['opt_dict']['tmp_dir'], file=os.path.basename(freeze_export_file))

    file_handle = open(freeze_export_tmp, 'wb')
    file_handle.write('Top Ten Summary for multiple vm-support bundles...')
    pat_omitkey = re.compile(r'(id|raw|latency|time|asense)')
    ...
    for category in ('iofails', 'sioclmt', 'latency', 'apdpdls'):
        ...
        for key in key_list:
            if not pat_omitkey.match(key):
                result = []
                sql_query = 'SELECT {field}, COUNT(*) c FROM {table} WHERE category=\'{cat}\' AND date<>\'1970-01-01\' GROUP BY {field} ORDER BY c DESC, {field} LIMIT 10'.format(field=key, cat=category, table='storage')

                try:
                    result = DB.query(sql_query)
                except RuntimeError:
                    RUNTIME_LOG.error('Failed dataset query for top ten summary {field} values - SQL "{sql}"'.format(field=key, sql=sql_query))
                if result:
...
def freeze_xlsx(freeze_setup):
    ...
    freeze_export_tmp = '{path}/{file}'.format(path=freeze_setup['opt_dict']['tmp_dir'], file=os.path.basename(freeze_setup['opt_dict']['xlsx_file']))
    ...
    for record in result:
        ...
        for key, value in record.items():
            ...
            if ('latency' or 'world' in key) and value.isdigit():
...
def freeze_xlsx_format(format_setup):
    ...
    for key in format_setup['wks_dict']['order'].keys():
        if 'date' in format_setup['wks_dict']['order'][key]:
            ...
            format_setup['worksheet'].set_column(alpha, format_setup['wks_dict']['widths'][format_setup['wks_dict']['order'][key]], format_date)
        elif 'latency' in format_setup['wks_dict']['order'][key]:
            tag_heatmap = key
            alpha = '{alpha}:{alpha}'.format(alpha=key)
            format_setup['worksheet'].set_column(alpha, format_setup['wks_dict']['widths'][format_setup['wks_dict']['order'][key]], format_number)
        else:
            alpha = '{alpha}:{alpha}'.format(alpha=key)
            format_setup['worksheet'].set_column(alpha, format_setup['wks_dict']['widths'][format_setup['wks_dict']['order'][key]])

    ## Format autofilter on columns date,time,host,dev,latency,raw
    format_setup['worksheet'].autofilter(0, 0, format_setup['wks_dict']['row_final'], format_setup['wks_dict']['col_final'])
...
def xlate_t10_sense(sense):
    ...
    if pat_hex.match(h_sense) and pat_hex.match(d_sense) and pat_hex.match(p_sense):
        ...
        RUNTIME_LOG.warning('Failed translating sense codes to T10 text - sense "H:{h} D:{d} P:{p}"'.format(h=h_sense, d=d_sense, p=p_sense))
        return sense
