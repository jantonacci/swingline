...
def cnvrt_msg(main_dict):
    ...
    for file_msg in main_dict['msg_list']:
        ...
        text_msg = '{u}.txt'.format(u=os.path.join(main_dict['dir_msg_txt'], hash_msg))
        subprocess.call([main_dict['msgtext_exe'], file_msg, text_msg],
                        stdout=file_log,
                        stderr=file_log)
        print '.',
        ...
        try:
            ...
            RUNTIME_LOG.error('Failed moving "{s}" to "{d}"'.format(s=file_msg,
...
def find_c3err(main_dict):
    """
    :desc  : Populate the main dictionary lists email and c3err
    :rtype : list
    """
